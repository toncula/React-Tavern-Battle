...
import { BookOpen, ArrowLeft, Sparkles, Compass, ChevronLeft, ChevronRight, Store, Coins, Trophy, Info, BrainCircuit, Ghost, Flame, Star, Zap, CloudRain, Sparkle, Search, Skull, Scroll, Swords, Gem } from 'lucide-react';
...
const Codex: React.FC<CodexProps> = ({ language, t, onBackToMenu, onInteraction, onHover, onLeave }) => {
  const [codexTab, setCodexTab] = useState<'CARDS' | 'ENEMIES' | 'ADVENTURE' | 'RULES'>('CARDS');
  ...
  const tabs = [
      ...
      { id: 'ADVENTURE', label: t.ui.tab_adventure, icon: Compass },
  ...
  const getTabInfo = () => {
    switch (codexTab) {
        ...
        case 'ADVENTURE': return { title: t.ui.tab_adventure, icon: Compass, desc: t.codex_section.intro };
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col font-sans selection:bg-cyan-500/30" onClick={onInteraction}>
        ...
        <div className="flex-1 overflow-y-auto bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-fixed">
            <div className="max-w-7xl mx-auto w-full p-4 md:p-8 min-h-full">
                ...
                {codexTab === 'ADVENTURE' && (
                     <div className="animate-in fade-in slide-in-from-bottom-4 max-w-6xl mx-auto space-y-12">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            {/* The Omen Section */}
                            <div className="bg-slate-900/60 border border-slate-800 p-8 rounded-3xl relative overflow-hidden group hover:border-slate-700 transition-colors shadow-2xl">
                                ...
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h3 className="text-2xl font-bold text-white uppercase tracking-wide">{t.adventure.the_omen}</h3>
                                </div>

                                <p className="text-slate-300 leading-relaxed mb-8 border-l-2 border-purple-500/30 pl-4 italic">
                                    {t.codex_section.oracle_intro}
                                </p>

                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="bg-slate-950 p-4 rounded-xl border border-amber-900/30 hover:border-amber-500/30 transition-colors group/item">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Star className="text-amber-400 fill-amber-400" size={16} />
                                            <span className="text-xs font-black text-amber-200 uppercase tracking-widest">{t.codex_section.mood_super_good}</span>
                                        </div>
                                        <p className="text-[10px] text-slate-500 group-hover/item:text-slate-400">{t.codex_section.mood_super_good_desc}</p>
                                    </div>

                                    <div className="bg-slate-950 p-4 rounded-xl border border-green-900/30 hover:border-green-500/30 transition-colors group/item">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Sparkles className="text-green-400" size={16} />
                                            <span className="text-xs font-black text-green-200 uppercase tracking-widest">{t.codex_section.mood_good}</span>
                                        </div>
                                        <p className="text-[10px] text-slate-500 group-hover/item:text-slate-400">{t.codex_section.mood_good_desc}</p>
                                    </div>

                                    <div className="bg-slate-950 p-4 rounded-xl border border-slate-700/30 hover:border-slate-500/30 transition-colors group/item">
                                        <div className="flex items-center gap-2 mb-2">
                                            <CloudRain className="text-slate-400" size={16} />
                                            <span className="text-xs font-black text-slate-200 uppercase tracking-widest">{t.codex_section.mood_moderate}</span>
                                        </div>
                                        <p className="text-[10px] text-slate-500 group-hover/item:text-slate-400">{t.codex_section.mood_moderate_desc}</p>
                                    </div>

                                    <div className="bg-slate-950 p-4 rounded-xl border border-red-900/30 hover:border-red-500/30 transition-colors group/item">
                                        <div className="flex items-center gap-2 mb-2">
                                            <Flame className="text-red-500" size={16} />
                                            <span className="text-xs font-black text-red-200 uppercase tracking-widest">{t.codex_section.mood_bad}</span>
                                        </div>
                                        <p className="text-[10px] text-slate-500 group-hover/item:text-slate-400">{t.codex_section.mood_bad_desc}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Rarity Section */}
                            <div className="bg-slate-900/60 border border-slate-800 p-8 rounded-3xl relative overflow-hidden group hover:border-slate-700 transition-colors shadow-2xl">
                                <div className="absolute -bottom-10 -left-10 opacity-5 group-hover:opacity-10 transition-opacity rotate-12 pointer-events-none">
                                    <Sparkle size={200} />
                                </div>
                                ...
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h3 className="text-2xl font-bold text-white uppercase tracking-wide">{t.ui.adventure_rarity_title}</h3>
                                </div>
                                
                                <p className="text-slate-300 leading-relaxed mb-8 border-l-2 border-yellow-500/30 pl-4">
                                    {t.ui.adventure_rarity_desc}
                                </p>
                                
                                <div className="space-y-4 relative z-10">
                                    <div className="bg-slate-950/80 p-5 rounded-2xl border border-slate-800/50 hover:border-slate-600 transition-all flex items-start gap-4">
                                        <div className="w-2 h-2 rounded-full bg-slate-500 mt-2 shrink-0 shadow-[0_0_8px_#64748b]" />
                                        <div>
                                            <div className="text-xs font-black text-slate-400 uppercase mb-1 tracking-widest">{t.ui.rarity_common}</div>
                                            <p className="text-sm text-slate-500 leading-snug">{t.ui.rarity_common_desc}</p>
                                        </div>
                                    </div>
                                    
                                    <div className="bg-slate-950/80 p-5 rounded-2xl border border-blue-900/30 hover:border-blue-500/50 transition-all flex items-start gap-4 shadow-[inset_0_0_20px_rgba(59,130,246,0.05)]">
                                        <div className="w-2 h-2 rounded-full bg-blue-500 mt-2 shrink-0 shadow-[0_0_8px_#3b82f6]" />
                                        <div>
                                            <div className="text-xs font-black text-blue-400 uppercase mb-1 tracking-widest">{t.ui.rarity_rare}</div>
                                            <p className="text-sm text-slate-500 leading-snug">{t.ui.rarity_rare_desc}</p>
                                        </div>
                                    </div>

                                    <div className="bg-slate-950/80 p-5 rounded-2xl border border-amber-900/30 hover:border-amber-500/50 transition-all flex items-start gap-4 shadow-[inset_0_0_20px_rgba(245,158,11,0.05)]">
                                        <div className="w-2 h-2 rounded-full bg-amber-500 mt-2 shrink-0 shadow-[0_0_8px_#f59e0b]" />
                                        <div>
                                            <div className="text-xs font-black text-amber-400 uppercase mb-1 tracking-widest">{t.ui.rarity_legendary}</div>
                                            <p className="text-sm text-slate-500 leading-snug">{t.ui.rarity_legendary_desc}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
